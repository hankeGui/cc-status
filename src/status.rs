...
pub fn run() -> Result<()> {
    ...
    let transcript_path: Option<PathBuf> = stdin
        ...
        .and_then(|v| v.as_str())
        .map(PathBuf::from)
        .or_else(|| cwd_for_lookup.as_deref().and_then(transcript::find_latest_for_cwd));
    ...
    if let (Some(p), true) = (ctx_pct, used > 0 && ctx_pct.map_or(false, |p| p < 100.0 && p >= 0.0)) {
        ...
        let used_pct = (used as f64 / (physical * pct_override / 100.0) * 100.0).clamp(0.0, 100.0) as i64;
        ...
        println!("│   CC 报剩余  {}%  {}(没有上一轮 token 数据，无法反推容量){}", pct_i, DIM, RESET);
    } else {
        println!("│   剩余       {}(无数据){}", DIM, RESET);
    }

    // --- 上一轮 ---
    println!("{B}│ 上一轮（最近一次 assistant 回复）{R}", B = BOLD, R = RESET);
    ...
    if lt_total == 0 {
        ...
        let total_in = sess.last_turn_input + sess.last_turn_cache_read + sess.last_turn_cache_creation;
        ...
        println!("│   发送 input  {} tokens（含 cache hit）", short_num(total_in));
        println!("│     ├ 命中缓存 {} {}({}× 折扣价){}", short_num(sess.last_turn_cache_read), DIM, "0.1", RESET);
        println!("│     ├ 写入缓存 {} {}(1.25× 贵价，下次能命中){}", short_num(sess.last_turn_cache_creation), DIM, RESET);
        println!("│     └ 新 input  {} {}(普通 input 价){}", short_num(sess.last_turn_input), DIM, RESET);
    ...
    if let Some(ms) = sess.last_cache_read_ms {
        ...
        if elapsed_s >= ttl_s {
            ...
            println!("│   剩余       {}{}:{:02}{}  {}{}{}", color, mm, ss, RESET, DIM, hint, RESET);
        }
    } else {
    ...
    if sess.total_input + sess.total_output + sess.total_cache_read + sess.total_cache_creation == 0 {
        ...
        println!("│   总 input    {} tokens", short_num(sess.total_input + sess.total_cache_read + sess.total_cache_creation));
        ...
        if let (Some(first), Some(last)) = (sess.first_turn_ms, sess.last_turn_ms) {
            ...
            if elapsed_ms >= 1000 {
                let total = sess.total_input + sess.total_output + sess.total_cache_read + sess.total_cache_creation;
...
fn color_for_hit(hit: u32) -> &'static str {
    if hit >= 70 { GREEN } else if hit >= 30 { YELLOW } else { RED }
}
